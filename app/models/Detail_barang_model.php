...
class Detail_barang_model
{
    ...
    public function getDataBarangByFilters($id_merek_barang, $id_jenis_barang, $id_lokasi)
    {
        ...
        if (!empty($id_merek_barang)) $query .= " AND b.id_merek_barang = :id_merek_barang";
        if (!empty($id_jenis_barang)) $query .= " AND b.id_jenis_barang = :id_jenis_barang";
        if (!empty($id_lokasi)) $query .= " AND b.id_lokasi_penyimpanan = :id_lokasi";
    ...
    public function hapusBarang($id_barang) {
        ...
        $res = $this->db->single();
        if ($res) {
            if (file_exists($res['foto_barang'])) unlink($res['foto_barang']);
            if (file_exists($res['qr_code'])) unlink($res['qr_code']);
        }
    ...
    public function ubahBarang($data) {
        ...
        $row = $this->db->single();
        if ($row && file_exists($row['qr_code'])) unlink($row['qr_code']);
    ...
    public function hapusDataMaster($table, $id)
    {
        ...
        switch ($table) {
            case 'mst_jenis_barang':
                $pkColumn = 'id_jenis_barang'; $fkColumn = 'id_jenis_barang'; break;
            case 'mst_merek_barang':
                $pkColumn = 'id_merek_barang'; $fkColumn = 'id_merek_barang'; break;
            case 'mst_lokasi_penyimpanan':
                $pkColumn = 'id_lokasi_penyimpanan'; $fkColumn = 'id_lokasi_penyimpanan'; break;
            case 'mst_status':
                $pkColumn = 'id_status'; $fkColumn = 'id_status'; break;
            case 'mst_satuan':
                $pkColumn = 'id_satuan'; $fkColumn = 'id_satuan'; break;
            default:
                return 0;
        }
